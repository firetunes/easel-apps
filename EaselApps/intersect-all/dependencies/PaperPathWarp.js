var PathWarp = function() {
	...
	*/
	...
	const warpBetween = function _warpBetween(topPath, bottomPath, options) {
	  return warpPathBetween(this, topPath, bottomPath, options);
	};
	...
	*/
	var warpPathBetween = function(targetPath, topPath, bottomPath, options) {
		const { smoothToleranceDeg } = options || {};
		let { flattenTolerance } = options || {};
		if (!flattenTolerance) {
		  flattenTolerance = 0.2;
		}
		const projection = dualBoundsPathProjection(topPath, bottomPath);
		...
		const transform = (path) => {
		  ...
		  //smoothCurves(path, smoothToleranceDeg);
		};
	...
	*/
	function dualBoundsPathProjection(topPath, bottomPath) {
	...
	*/
	function smoothCurves(path, toleranceDeg = 30) {
