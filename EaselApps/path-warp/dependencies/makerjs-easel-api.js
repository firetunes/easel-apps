...
function exportModelToEaselPointArray(model) {
    ...
    makerjs.model.findChains(model, function (chains, loose, layer) {
        ...
        chains.forEach(function (chain) {
			...
			/*
			curchain_model = makerjs.chain.toNewModel(chain, false);
			curExtents = makerjs.measure.modelExtents(curchain_model);
			console.log(prevExtents);
			if (prevExtents != null){
				// check if chain needs to be reversed
				if((((curExtents.center[0] - curExtents.width / 2) > (prevExtents.center[0] - prevExtents.width / 2)) &&
					((curExtents.center[0] + curExtents.width / 2) < (prevExtents.center[0] + prevExtents.width / 2))) &&
					(((curExtents.center[1] - curExtents.height / 2) > (prevExtents.center[1] - prevExtents.height / 2)) &&
					((curExtents.center[1] + curExtents.height / 2) < (prevExtents.center[1] + prevExtents.height / 2)))) {
						// curModel is inside prev model
						makerjs.chain.reverse(chain);
				} else {
					prevchain_model = makerjs.chain.toNewModel(chain, false);
					prevExtents = makerjs.measure.modelExtents(curchain_model);
				}
			} else {
				prevchain_model = makerjs.chain.toNewModel(chain, false);
				prevExtents = makerjs.measure.modelExtents(curchain_model);
